...
# -------------------------------------------------------------------
# 1. "BANCO DE DADOS FALSO" - Caminhos de imagem relativos
# -------------------------------------------------------------------
...
CPUS = {
    "1": { 
        ...
        "socket": "AM4", 
        "ram_support_types": ["DDR4"], 
        "tier": 7,
        "tdp": 65, 
        "image_url": "static/images/amd-ryzen-5-5600x.jpg" # Caminho relativo
    },
    "2": { 
        "id": "2", 
        "name": "Intel Core i5-13600K", 
        "socket": "LGA1700", 
        "ram_support_types": ["DDR4", "DDR5"], 
        "tier": 9,
        "tdp": 125, 
        "image_url": "static/images/intel-i5-13600k.jpg" # Caminho relativo
    }
}
MOTHERBOARDS = {
    "101": { 
        ...
        "socket": "AM4", 
        "ram_type": "DDR4",
        "image_url": "static/images/asus-tuf-b550m-plus.jpg" # Caminho relativo
    },
    "102": { 
        "id": "102", 
        ...
        "socket": "LGA1700", 
        "ram_type": "DDR5",
        "image_url": "static/images/gigabyte-z790-aorus.jpg" # Caminho relativo
    },
    "103": { 
        "id": "103", 
        ...
        "socket": "LGA1700", 
        "ram_type": "DDR4",
        "image_url": "static/images/asrock-b660m-steel-legend.jpg" # Caminho relativo
    }
}
RAM_MODULES = {
    "201": { 
        ...
        "type": "DDR4",
        "image_url": "static/images/corsair-vengeance-16gb-ddr4.jpg" # Caminho relativo
    },
    "202": { 
        "id": "202", 
        "name": "Kingston Fury 32GB (DDR5)", 
        "type": "DDR5",
        "image_url": "static/images/kingston-fury-32gb-ddr5.jpg" # Caminho relativo
    }
}
GPUS = {
    "301": { 
        ...
        "length_mm": 242, 
        "tdp": 170, 
        "image_url": "static/images/nvidia-rtx-3060.png" # Caminho relativo
    },
    "302": { 
        "id": "302", 
        ...
        "length_mm": 304, 
        "tdp": 320, 
        "image_url": "static/images/nvidia-rtx-4080.png" # Caminho relativo
    }
}
PSUS = {
    "401": { 
        ...
        "wattage": 650,
        "image_url": "static/images/corsair-cv650.jpg" # Caminho relativo
    },
    "402": { 
        "id": "402", 
        "name": "Corsair RM850x (850W)", 
        "wattage": 850,
        "image_url": "static/images/corsair-rm850x.jpg" # Caminho relativo
    }
}
COOLERS = {
    "501": { 
        "id": "501", 
        "name": "Cooler Box Original AMD", 
        "height_mm": 70, 
        "supported_sockets": ["AM4"],
        "image_url": "static/images/amd-cooler-box.jpg" # Caminho relativo
    },
    "502": { 
        "id": "502", 
        "name": "Deepcool AG400", 
        "height_mm": 150, 
        "supported_sockets": ["AM4", "AM5", "LGA1700", "LGA1200"],
        "image_url": "static/images/deepcool-ag400.jpg" # Caminho relativo
    },
    "503": { 
        "id": "503", 
        ...
        "image_url": "static/images/corsair-h100i-wc.jpg" # Caminho relativo
    }
}
CASES = { 
    "601": { 
        "id": "601", 
        "name": "Gabinete Gamer Mancer Goblin", 
        "gpu_max_length_mm": 280, 
        "cooler_max_height_mm": 160,
        "image_url": "static/images/mancer-goblin.jpg" # Caminho relativo
    },
    "602": { 
        "id": "602", 
        "name": "Gabinete Corsair 4000D Airflow", 
        "gpu_max_length_mm": 360, 
        "cooler_max_height_mm": 170,
        "image_url": "static/images/corsair-4000d-airflow.jpg" # Caminho relativo
    }
}

# (Dicionários REPAIR_SHOPS e PREBUILTS completos continuam iguais)
REPAIR_SHOPS = {
...
VERIFIED_SHOPS = {
    "2001": { "id": "2001", "name": "Reparo Justo PC", "type": "pc", "address": "Av. Confiança, 10 - Centro", "phone": "(21) 90000-1111" },
    "2002": { "id": "2002", "name": "Celular Honesto", "type": "celular", "address": "Rua da Garantia, 20 - Lapa", "phone": "(21) 90000-2222" },
    "2003": { "id": "2003", "name": "InfoFiel", "type": "pc", "address": "Rua do Técnico, 30", "phone": "(21) 90000-3333" }
}
PREBUILTS = {
...
# -------------------------------------------------------------------
# 2. ROTAS PARA BUSCAR AS LISTAS DE PEÇAS
# -------------------------------------------------------------------
@app.route('/api/get-cpus', methods=['GET'])
def get_cpus():
    return jsonify(list(CPUS.values()))

@app.route('/api/get-motherboards', methods=['GET'])
def get_motherboards():
    return jsonify(list(MOTHERBOARDS.values()))

@app.route('/api/get-ram', methods=['GET'])
def get_ram():
    return jsonify(list(RAM_MODULES.values()))

@app.route('/api/get-gpus', methods=['GET'])
def get_gpus():
    return jsonify(list(GPUS.values()))

@app.route('/api/get-psus', methods=['GET'])
def get_psus():
    return jsonify(list(PSUS.values()))

@app.route('/api/get-coolers', methods=['GET'])
def get_coolers():
    return jsonify(list(COOLERS.values()))
...
def get_prebuilts():
    return jsonify(list(PREBUILTS.values()))

# -------------------------------------------------------------------
# 3. "MOTOR DE REGRAS" (COMPLETO E CORRETO)
# -------------------------------------------------------------------
...
def check_build():
    ...
    if cpu and mobo:
        if cpu['socket'] != mobo['socket']:
            incompatibilities.append(f"INCOMPATÍVEL: A CPU {cpu['name']} (soquete {cpu['socket']}) não é compatível com a placa-mãe {mobo['name']} (soquete {mobo['socket']}).")

    if ram and mobo:
        if ram['type'] != mobo['ram_type']:
            incompatibilities.append(f"INCOMPATÍVEL: A RAM {ram['name']} ({ram['type']}) não é compatível com a placa-mãe {mobo['name']} (requer {mobo['ram_type']}).")

    if cpu and mobo:
        if 'ram_support_types' in cpu and cpu['ram_support_types']:
             if mobo['ram_type'] not in cpu['ram_support_types']:
                other_warnings.append(f"AVISO: A placa-mãe {mobo['name']} usa {mobo['ram_type']}, que não é suportado nativamente pelo controlador de memória da CPU {cpu['name']}.")

    # Regras de Gargalo e Fonte
    if psu and gpu:
        if 'wattage' in psu and 'min_psu_watts' in gpu:
            if psu['wattage'] < gpu['min_psu_watts']:
                other_warnings.append(f"AVISO DE FONTE: A fonte {psu['name']} ({psu['wattage']}W) está abaixo do recomendado de {gpu['min_psu_watts']}W para a GPU {gpu['name']}.")

    if cpu and gpu:
        if 'tier' in cpu and 'tier' in gpu:
            if gpu['tier'] >= (cpu['tier'] + 3):
                bottlenecks.append(f"RISCO DE GARGALO: A GPU {gpu['name']} (Nível {gpu['tier']}) é mais potente que a CPU {cpu['name']} (Nível {cpu['tier']}).")
            elif cpu['tier'] >= (gpu['tier'] + 3):
                bottlenecks.append(f"RISCO DE GARGALO: A CPU {cpu['name']} (Nível {cpu['tier']}) é mais potente que a GPU {gpu['name']} (Nível {gpu['tier']}).")

    # Regras Físicas
    if cooler and mobo:
        if 'socket' in mobo and 'supported_sockets' in cooler:
            if mobo['socket'] not in cooler['supported_sockets']:
                incompatibilities.append(f"INCOMPATÍVEL: O cooler {cooler['name']} não suporta o soquete {mobo['socket']} da placa-mãe.")

    if gpu and case:
        if 'length_mm' in gpu and 'gpu_max_length_mm' in case:
            if gpu['length_mm'] > case['gpu_max_length_mm']:
                incompatibilities.append(f"INCOMPATÍVEL: A GPU {gpu['name']} ({gpu['length_mm']}mm) não cabe no gabinete {case['name']} (limite: {case['gpu_max_length_mm']}mm).")

    if cooler and case:
        if 'height_mm' in cooler and 'cooler_max_height_mm' in case:
            if cooler['height_mm'] > case['cooler_max_height_mm']:
                incompatibilities.append(f"INCOMPATÍVEL: O cooler {cooler['name']} ({cooler['height_mm']}mm) é muito alto para o gabinete {case['name']} (limite: {case['cooler_max_height_mm']}mm).")

    response = {
        'build_is_compatible': len(incompatibilities) == 0,
...
# -------------------------------------------------------------------
# 4. ROTAS DE ASSISTÊNCIA TÉCNICA
# -------------------------------------------------------------------
@app.route('/api/get-pc-shops', methods=['GET'])
def get_pc_shops():
    pc_shops_list = [shop for shop in REPAIR_SHOPS.values() if shop['type'] == 'pc']
    return jsonify(pc_shops_list)

@app.route('/api/get-celular-shops', methods=['GET'])
def get_celular_shops():
    celular_shops_list = [shop for shop in REPAIR_SHOPS.values() if shop['type'] == 'celular']
    return jsonify(celular_shops_list)

@app.route('/api/get-verified-shops', methods=['GET'])
def get_verified_shops():
    return jsonify(list(VERIFIED_SHOPS.values()))

# -------------------------------------------------------------------
# 5. ROTA PARA CALCULAR CONSUMO
# -------------------------------------------------------------------
...
def calculate_wattage():
    ...
    response = {
        'estimated_wattage': total_estimated_tdp,
        'recommended_wattage': recommended_psu
    }
    return jsonify(response)
